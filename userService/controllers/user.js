...
const verifyToken = async (req, res) => {
  try {
    ...
    if (!payload || !payload.userId) {
      return res
        .status(401)
        .json({ error: { message: "Invalid token payload" } });
    }
...
const signIn = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, accessToken, user: userResult });
  } catch (error) {
    console.log(error);
    next(error);
  }
...
const signUp = async (req, res, next) => {
  ...
  try {
    ...
    if (userByEmail) {
      return res
        .status(401)
        .json({ code: 401, message: "Email is already registered." });
    }
...
const sendOTP = async (req, res, next) => {
  try {
    ...
    if (user.type !== "local") {
      return res
        .status(400)
        .json({ message: "Please use Google to change your password." });
    }
  ...
  try {
    ...
    const userResult = {
      ...
      birthOfDate: user.birthOfDate
        ? user.birthOfDate.toISOString().split("T")[0]
        : "",
...
const editUserById = async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    const { firstName, lastName, email, phone, birthOfDate, address } =
      req.body.data;
...
const changeAVT = async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while changing the avatar" });
  }
};
